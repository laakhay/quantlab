...
from laakhay.quantlab.backend import (
    backend, has_backend, list_backends, ArrayBackend,
    register, get_backend, set_default, infer_backend, 
    infer_backend_from_arrays, convert_array
)
...
class TestBackendRegistry:
    ...
    def test_has_backend(self):
        ...
        assert has_backend('fallback') == False  # fallback doesn't count
...
class TestArrayBackendWrapper:
    ...
    def test_supports_autodiff(self):
        ...
        if has_backend('jax'):
            b = ArrayBackend('jax')
            assert b.supports_autodiff == True
        ...
        if has_backend('numpy'):
            ...
            assert b.supports_autodiff == False
...
class TestAutodiff:
    ...
    def test_grad_scalar_function(self, b_autodiff):
        ...
        def f(x):
            ...
            return b_autodiff.add(
                b_autodiff.add(b_autodiff.mul(x, x), b_autodiff.mul(3, x)),
                1
            )
    ...
    def test_jit(self, b_autodiff):
        ...
        np.testing.assert_allclose(
            b_autodiff.to_numpy(result_normal),
            b_autodiff.to_numpy(result_jit)
        )
