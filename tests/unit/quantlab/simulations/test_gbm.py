...
class TestGeometricBrownianMotion:
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(ValueError, match="Spot price must be positive"):
            GeometricBrownianMotion(spot=0.0, drift=0.05, volatility=0.2, 
                                   time_to_maturity=1.0, num_steps=100)
        
        with pytest.raises(ValueError, match="Volatility must be non-negative"):
            GeometricBrownianMotion(spot=100.0, drift=0.05, volatility=-0.2, 
                                   time_to_maturity=1.0, num_steps=100)
        
        with pytest.raises(ValueError, match="Time to maturity must be positive"):
            GeometricBrownianMotion(spot=100.0, drift=0.05, volatility=0.2, 
                                   time_to_maturity=0.0, num_steps=100)
        
        with pytest.raises(ValueError, match="Number of steps must be positive"):
            GeometricBrownianMotion(spot=100.0, drift=0.05, volatility=0.2, 
                                   time_to_maturity=1.0, num_steps=0)
    ...
    def test_generate_shocks_statistics(self, backend):
        ...
        gbm = GeometricBrownianMotion(
            spot=100.0, drift=0.05, volatility=0.2,
            time_to_maturity=1.0, num_steps=252
        )
        num_paths = 10000
    ...
    def test_terminal_values_statistics(self, backend):
        ...
        gbm = GeometricBrownianMotion(
            spot=spot, drift=risk_free_rate, volatility=volatility,
            time_to_maturity=time_to_maturity, num_steps=1
        )
        num_paths = 100000
        ...
        expected_mean = spot * float(backend.to_numpy(backend.exp(backend.array(risk_free_rate * time_to_maturity))))
    ...
    def test_terminal_values_antithetic(self, backend):
        ...
        gbm = GeometricBrownianMotion(
            spot=100.0, drift=0.05, volatility=0.2,
            time_to_maturity=1.0, num_steps=1
        )
        num_paths = 10000
        
        if backend.name == "jax":
            key = backend.random_key(42)
            terminal_vals = gbm.terminal_values(num_paths, backend=backend, key=key, antithetic=True)
        else:
    ...
    def test_add_drift(self, backend):
        ...
        gbm = GeometricBrownianMotion(
            spot=100.0, drift=0.1, volatility=0.2,
            time_to_maturity=1.0, num_steps=100
        )
        ...
        expected_final = constant_value * float(backend.to_numpy(
