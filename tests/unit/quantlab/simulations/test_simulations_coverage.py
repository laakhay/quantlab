...
from laakhay.quantlab.simulations.samplers import BaseSampler, GaussianSampler
...
class TestSimulationsCoverage:
    ...
    def test_gaussian_sampler_all_methods(self):
        ...
        assert backend.shape(pdf_std) == (3,)
        ...
        assert all(0 <= float(backend.to_numpy(v)) <= 1 for v in cdf_std)

        # Test PPF
        q = backend.array([0.1, 0.5, 0.9])
        ppf_vals = sampler.ppf(q, backend=backend)
        assert backend.shape(ppf_vals) == (3,)

        # Test standardized PPF
        ppf_std = sampler.ppf(q, backend=backend, standardize=True)
        assert backend.shape(ppf_std) == (3,)

        # Test that moments properties exist (they are computed via sampling)
        assert isinstance(sampler.skew, float)  # Just access the property
        assert isinstance(sampler.ex_kurtosis, float)  # Just access the property
    ...
    def test_gbm_all_methods(self):
        ...
        assert backend.shape(drifted) == (100, 253)
