...
def test_repro_sl_exits_trade():
    ...
    config = BacktestConfig(
        frequency=FrequencyControl(max_entries_per_day=10)
    )
...
def test_stuck_trade_blocking_new_entry():
    ...
    # We expect:
    # Trade 1: Entry @ 100 (idx 0), Exit @ 90 SL (idx 2).
    # Trade 2: Entry @ 100 (idx 3 or 4).
    
    # If Trade 1 is stuck, we see only 1 trade (or 0 closed).
    assert len(trades) >= 2, f"Expected 2 trades, got {len(trades)}. Stuck trade might be blocking re-entry."
