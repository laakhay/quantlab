...
def compute_d1_d2(
    strike: float, expiry: float, market: MarketData, drift: Union[object, None] = None
) -> tuple[object, object]:
    ...
    strike_arr = backend.convert(strike)
    expiry_arr = backend.convert(expiry_val)
...
def compute_discount_factor(expiry: float, market: MarketData) -> object:
    ...
    backend = market.backend
    expiry_arr = backend.convert(expiry)
    return backend.exp(backend.mul(backend.mul(-1, market.rate), expiry_arr))


def compute_barrier_terms(option, market: MarketData) -> dict[str, object]:
    ...
    mirror_factor = backend.power(h_over_s, backend.mul(2.0, mu))

    N_d1, N_d2 = backend.norm_cdf(d1), backend.norm_cdf(d2)
    N_d3, N_d4 = backend.norm_cdf(d3), backend.norm_cdf(d4)
    N_d5, N_d6 = backend.norm_cdf(d5), backend.norm_cdf(d6)
    N_d7, N_d8 = backend.norm_cdf(d7), backend.norm_cdf(d8)

    discount = compute_discount_factor(T, market)

    A = backend.add(backend.mul(market.spot, N_d1), backend.mul(backend.mul(-K, discount), N_d2))

    B = backend.add(backend.mul(market.spot, N_d3), backend.mul(backend.mul(-K, discount), N_d4))

    reflection_spot = backend.mul(market.spot, mirror_factor)
    reflection_strike = backend.mul(
        backend.mul(K, discount), backend.power(h_over_s, backend.add(backend.mul(2.0, mu), -2.0))
    )
    C = backend.add(
        backend.mul(reflection_spot, N_d5), backend.mul(backend.mul(-1.0, reflection_strike), N_d6)
    )

    D = backend.add(
        backend.mul(reflection_spot, N_d7), backend.mul(backend.mul(-1.0, reflection_strike), N_d8)
    )
...
def compute_asian_adjusted_params(market: MarketData) -> tuple[object, object]:
    ...
    vol_hat = backend.divide(market.vol, backend.sqrt(backend.convert(3.0)))
