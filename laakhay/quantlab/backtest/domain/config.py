...
class PortfolioControl:
    """Portfolio-level constraints for multi-asset runs."""

    mode: PortfolioMode = PortfolioMode.INDEPENDENT
    max_open_positions: int | None = None
    max_symbol_weight_pct: float | Decimal | None = None
    max_gross_exposure_pct: float | Decimal | None = None
    max_net_exposure_pct: float | Decimal | None = None

    def __post_init__(self) -> None:
        if self.max_open_positions is not None and self.max_open_positions <= 0:
            raise ValueError("max_open_positions must be > 0 when provided")
...
class LeverageControl:
    """Leverage and margin placeholders for future portfolio margining."""

    enabled: bool = False
    leverage: float | Decimal = 1.0
    margin_mode: MarginMode = MarginMode.CROSS
    maintenance_margin_ratio: float | Decimal | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.maintenance_margin_ratio is not None:
            mmr = Decimal(str(self.maintenance_margin_ratio))
            if mmr <= 0 or mmr >= 1:
...
class BacktestConfig:
    ...
    portfolio: PortfolioControl = field(default_factory=PortfolioControl)
    leverage: LeverageControl = field(default_factory=LeverageControl)
    risk_free_rate: float = 0.0
    protective_exit_policy: ProtectiveExitPolicy = ProtectiveExitPolicy.CONSERVATIVE
